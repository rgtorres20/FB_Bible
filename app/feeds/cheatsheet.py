...
def build_html(state: dict, index: dict | None, now: datetime) -> str:
    ...
    if not board:
        return (
            skin.head("cheat sheet", "Cheat sheet", _STYLE) + "<h1>Draft cheat sheet</h1>"
    ...
    return (
        skin.head("cheat sheet", "Cheat sheet", _STYLE)
        + "<h1>Draft cheat sheet — NDDPL (10tm) &amp; RED_EYE (12tm)</h1>"
        f"<p class='sub'>Live ADP from real PPR mock drafts — read the 10tm column "
        f"for NDDPL and the 12tm column for RED_EYE · ★ = position-adjusted "
        f"sleeper find · generated {html.escape(stamp)} · "
